...
from forms import LoginForm, RateEditForm, RateForm, MovieForm
...
def edit_rating(id):
    ...
    # Get the movie or show 404 if not found
    ratings = Rating.query.get_or_404(id)
    # Load current movie details into the form
    form = RateEditForm(obj=ratings)
    ...
    if form.validate_on_submit():
        # Update movie info
        ratings.name = form.name.data
        ratings.movie = form.movieList.data
        ratings.stars = form.stars.data
        ratings.remarks = form.remarks.data
        db.session.commit()
        flash('Movie updated!', 'success')
